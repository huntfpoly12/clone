...
const employeeList = computed(() => {
  ...
  const isJoinedBeforeSelectedMonth = (item: any) => +item.joinedAt.toString().slice(0, 4) <= selectMonthColumn.value.imputedYear && +item.joinedAt.toString().slice(4, 6) <= selectMonthColumn.value.imputedMonth && !employeeIdCreated.value?.find((i:any) => i.employeeId === item.employeeId && i.employeeType === item.type)
  const isLeavedAfterSelectedMonth = (item: any) => +item.leavedAt.toString().slice(0, 4) >= selectMonthColumn.value.imputedYear && +item.leavedAt.toString().slice(4, 6) >= selectMonthColumn.value.imputedMonth && !employeeIdCreated.value?.find((i:any) => i.employeeId === item.employeeId && i.employeeType === item.type);
