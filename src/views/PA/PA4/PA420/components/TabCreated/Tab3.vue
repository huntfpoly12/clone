      ...
      <a-row :gutter="16" class="modal-add modal-add-step-3">
          ...
          <a-form-item label="과세대상 퇴직급여(확정)">
            <div class="d-flex-center">
              <div :class="taxableRetirementBenefitsRef <= 0 ? 'validate-caculate' : ''">
                <number-box-money :required="false" width="150px" :value-input="taxableRetirementBenefitsRef"
        ...
        <a-col :span="12">
          ...
          <div class="d-flex">
            ...
            <biz-number-text-box v-model:valueInput="statementsAfterCal1.bizNumber" disabled :required="false"
                                 width="20%"
          ...
          <div class="d-flex mt-5 mb-5">
            <default-text-box v-model:valueInput="statementsAfterCal2.pensionAccountHolder" disabled :required="false"
                              width="20%" class="mr-5" placeholder="연금계좌취급자"/>
            <biz-number-text-box v-model:valueInput="statementsAfterCal2.bizNumber" disabled :required="false"
                                 width="20%"
                                 class="mr-5" placeholder="사업자등록번호"/>
...
onResult((value) => {
  ...
  if (value.data.calculateIncomeRetirementTax) {
    const result = value.data.calculateIncomeRetirementTax.calculationOfDeferredRetirementIncomeTax
    ...
    dataIncomeRetirementTax.value = cloneDeep(result)
    if (value.data.calculateIncomeRetirementTax?.calculationOfDeferredRetirementIncomeTax?.statements.length > 0) {
    ...
    // if (!value.data.calculateIncomeRetirementTax?.calculationOfDeferredRetirementIncomeTax) {
    //   dataIncomeRetirementTax.value.calculationOfDeferredRetirementIncomeTax.totalAmount = 0
    // }
  }
...
const handleCalculateIncomeRetirementTax = () => {
  ...
  if (isReqStatements1.value && !formState.calculationOfDeferredRetirementIncomeTax.statements[0].depositDate) {
  ...
  if (isReqStatements2.value && !formState.calculationOfDeferredRetirementIncomeTax.statements[1].depositDate) {
  ...
  const res = tab3AddForm.value.validate()
  ...
  if (!res.isValid) {
    ...
    if (!result.calculationOfDeferredRetirementIncomeTax?.statements?.length) delete result.calculationOfDeferredRetirementIncomeTax
    variables.value = {
      companyId: companyId,
      input: result
    }
...
function compareObjects(obj1: any, obj2: any) {
  ...
  if (count === keys1.length || count === 0) return false
  return true;
}
