...
const taxCalculationInputStore = computed(
  () => store.getters["common/getTaxCalculationInput"]
);
...
const statementsAfterCal1 = ref({
  ...initialIncomeRetirementTax.calculationOfDeferredRetirementIncomeTax
    .statements[0],
});
const statementsAfterCal2 = ref({
  ...initialIncomeRetirementTax.calculationOfDeferredRetirementIncomeTax
    .statements[0],
});
...
onResult((value) => {
  ...
  if (value.data.calculateIncomeRetirementTax) {
    ...
    if (
      ...
      statementsAfterCal1.value =
        value.data.calculateIncomeRetirementTax.calculationOfDeferredRetirementIncomeTax.statements[0];
    }
    if (
      value.data.calculateIncomeRetirementTax
        ?.calculationOfDeferredRetirementIncomeTax?.statements.length > 1
    ) {
      statementsAfterCal2.value =
        value.data.calculateIncomeRetirementTax.calculationOfDeferredRetirementIncomeTax.statements[1];
    }
...
const handleCalculateIncomeRetirementTax = () => {
  ...
  if (!res.isValid) {
    ...
    if (interimPaymentTab1.value) {
      ...
      if (
        taxCalculationInputStore.value.prevRetirementBenefitStatus
