<template>
	<div class="page-content">
		<div class="content-page2">
			<a-spin :spinning="loading" size="large">
				<div class="select-employee">
					<div class="label">사원</div>
					<div class="input-select">
						<!-- <employ-select :arrayValue="arrayEmployeeSelect" v-model:valueEmploy="valueSelected" width="300px"
							:disabled="true" /> -->
						<DxSelectBox :noDataText="Message.getMessage('COMMON', '901').message" :width="300"
							...
							field-template="field-data" class="custom-select" :dropDownOptions="{ minHeight: '80px' }"
							:disabled="true">
				...
				<div class="info-employee">
					<standard-form formName="formPA410" ref="formPA410">
							...
							<a-col :span=" 12 ">
								...
								<div class="input-employee">
									<a-form-item label="정산시작(가산)일" label-align="right" class="red">
										<date-time-box width="150px" v-model:valueDate=" formState.settlementStartDate "
											ref="payStart"></date-time-box>
									</a-form-item>
									<info-tool-tip placement="right">퇴직소득 정산의 시작일(기산일)로서, 중간정산지급 등으로
										인해 입사일과 상이할 수 있습니다.</info-tool-tip>
								</div>
								<div class="input-employee">
									<a-form-item label="정산종료(퇴사)일" label-align="right" class="red">
										<date-time-box-custom width="150px"
											v-model:valueDate=" formState.settlementFinishDate " :startDate="
												dayjs(String(formState.settlementStartDate)).add(
													1,
													'day'
												)
											" ref="payEnd"></date-time-box-custom>
									</a-form-item>
									<info-tool-tip placement="right">퇴직소득 정산의 종료일로서, 중간정산지급인 경우 퇴사일과
										상이할 수 있습니다.</info-tool-tip>
								</div>
								<div class="input-employee">
									<a-form-item label="제외일수(일)" label-align="right">
										<div class="d-flex-center">
											<number-box width="151px" :spinButtons=" false " :rtlEnabled=" true "
												v-model:valueInput=" formState.exclusionDays " format="0,###">
											</number-box>
										</div>
									</a-form-item>
									<info-tool-tip placement="right">정산시작(기산)일 기준 제외일수만큼 뒤로 미뤄서 근속일수를
										계산합니다.</info-tool-tip>
								</div>
								<div class="input-employee">
									<a-form-item label="가산일수(일)" label-align="right">
										<div class="d-flex-center">
											<number-box width="151px" :spinButtons=" false " :rtlEnabled=" true "
												v-model:valueInput=" formState.additionalDays " format="0,###">
											</number-box>
										</div>
									</a-form-item>
									<info-tool-tip placement="right">정산시작(기산)일 기준 가산일수만큼 앞으로 당겨서 근속일수를
										계산합니다.</info-tool-tip>
								</div>
							</a-col>
							<a-col :span=" 12 ">
								...
								<div class="salary">
									<div class="input-salary">
										<a-form-item label="퇴직전 3개월 임금 총액 (세전)" label-align="right" class="red">
											<number-box-money width="150px" :spinButtons=" false " :rtlEnabled=" true "
												v-model:valueInput=" formState.totalPay3Month " :required=" true "
												format="0,###">
											</number-box-money>
										</a-form-item>
									...
									<div class="input-salary">
										<a-form-item label="연간 상여금 총액" label-align="right" class="red">
											<number-box-money width="150px" :spinButtons=" false " :rtlEnabled=" true "
												v-model:valueInput=" formState.totalAnualBonus " :required=" true "
												format="0,###">
											</number-box-money>
										</a-form-item>
										<span class="pl-5 pt-4">원</span>
									...
									<div class="input-salary">
										<a-form-item label="연차수당" label-align="right" class="red">
											<number-box-money width="150px" :spinButtons=" false " :rtlEnabled=" true "
												v-model:valueInput=" formState.annualLeaveAllowance " :required=" true "
												format="0,###">
											</number-box-money>
										</a-form-item>
										<span class="pl-5 pt-4">원</span>
						...
						<div class="time-service">
							<a-tag color="red" style="margin-left: 10px; margin-right: 39px"><span
									style="color: black; font-size: 14px">근속연월일</span>
							</a-tag>
							<a-tag style="min-width: 150px; text-align: center">
								<span style="color: #c55a11; font-size: 14px; text-align: center">
									{{
									!Number.isNaN(dataLastRetiredYearsOfService.yearsOfService)
									? dataLastRetiredYearsOfService.yearsOfService
									: 0
									}}년
									{{
									!Number.isNaN(dataLastRetiredYearsOfService.monthsOfService)
									? dataLastRetiredYearsOfService.monthsOfService
									: 0
									}}개월
									{{
									!Number.isNaN(dataLastRetiredYearsOfService.daysOfService)
									? dataLastRetiredYearsOfService.daysOfService
									: 0
									}}일
								</span>
							</a-tag>
		...
		<div class="infor-text" style="text-align: center; margin-bottom: 20px; color: gray">
			상기 급여(수당)으로 퇴직금을 계산합니다.
		</div>
	</div>
</template>
...
export default defineComponent({
	components: { DxSelectBox, DxTextBox },
	setup(props, { emit }) {
		onMounted(() => {
			if (!isEmpty(formStateStep2.value)) {
				Object.assign(formState, formStateStep2.value);
			} else {
				let employeeInfor = store.state.common.arrayEmployeePA410.find(
					(item: any) => item.employeeId_Type == store.state.common.employeeIdPA410
				);
				formState.settlementStartDate =
					employeeInfor && employeeInfor.joinedAt
						? employeeInfor.joinedAt
						: filters.formatDateToInterger(dayjs().format("YYYY-MM-DD"));
				formState.settlementFinishDate =
					employeeInfor && employeeInfor.leavedAt
						? employeeInfor.leavedAt
						: null;
		...
		const formStateStep2 = computed(
			() => store.getters["common/stateStep2PA410"]
		);
		...
		const formState = reactive({
			...initFormState,
			settlementStartDate: filters.formatDateToInterger(
				dayjs().format("YYYY-MM-DD")
			),
			settlementFinishDate: filters.formatDateToInterger(
				dayjs().format("YYYY-MM-DD")
			),
		});
		let variables = {
		...
		const { result, loading, refetch, onError, onResult } = useQuery(
			queries.calculateIncomeRetirement,
			variables,
			() => ({
		...
		onResult((value) => {
			if (value.data) {
				store.commit(
					"common/setCaculateValuePA410",
					value.data.calculateIncomeRetirement
				);
				trigger.value = false;
