  ...
  <div id="pa-830" class="px-10 py-10">
    <a-spin :spinning="false" size="large">
      <DxDataGrid :show-row-lines="true" :hoverStateEnabled="true" :data-source="dataSource" :show-borders="true"
        ...
        <DxColumn caption="생년월일" data-field="residentId" cell-template="convertBirthday" width="100" alignment="center" />
        ...
        <DxColumn caption="급여변경신고다운로드" data-field="fileStorageId" cell-template="viewUrl" width="200"
          alignment="center" />
        <template #viewUrl="{ data }" class="custom-action">
          <div class="d-flex justify-content-center">
            <DxButton type="ghost" class="" style="cursor: pointer" @click="handleOpenViewReportUrl(data.data.workId)">
    ...
    <HistoryPopup :modalStatus="modalHistory" @closePopup="modalHistory = false" :data="actionParam.workId" title="변경이력"
      typeHistory="pa-830" />
    <CreatePA830Popup v-if="isOpenModalCreate" :isOpenModalCreate="isOpenModalCreate"
      @closeModal="isOpenModalCreate = false" @createDone="createDone" />
  </div>
</template>
...
const {
  onResult: OnResultReportViewUrl,
  onError: OnErrorReportViewUrl
} = useQuery(queries.getMajorInsuranceCompanyEmployeeSalaryChangeFaxFilingReportViewUrl, actionParam, () => ({
  fetchPolicy: 'no-cache',
  enabled: triggerReportViewUrl.value
}));
OnResultReportViewUrl(({ data }) => {
  window.open(data.getMajorInsuranceCompanyEmployeeSalaryChangeFaxFilingReportViewUrl, '_blank')
