  ...
  <standard-form action="" name="add-page-210" class="formPA510">
      ...
      <StandardForm formName="pa-510-form" ref="pa510FormRef">
        <a-row :key="countKey">
          <a-col :span="14">
            <a-form-item
              label="일용직사원"
              :class="store.state.common.pa510.statusFormAdd ? 'red' : ''"
            >
              <EmploySelect
                ...
                "
                v-model:valueEmploy="dataIncomeWageDaily.employee.employeeId"
                :required="true"
                @onChange="onChange"
                :activeType20="false"
                width="270px"
            ...
            <a-form-item label="지급일" class="red">
              ...
              <date-time-box-custom
                ...
                v-model:valueDate="dataIncomeWageDaily.paymentDay"
                :disabled="
                  !store.state.common.pa510.statusFormAdd ||
                  store.state.common.pa510.statusDisabledStatus
                "
          ...
          <a-col :span="10">
            ...
            <a-form-item label="근무일수">
              <number-box
                :disabled="true"
                v-model:valueInput="dataIncomeWageDaily.workingDays"
                width="160px"
                :required="true"
                min="1"
                :max="31"
            ...
            <a-form-item label="공제합계">
              <number-box-money
                :disabled="true"
                v-model:valueInput="dataIncomeWageDaily.totalDeduction"
                width="160px"
                :required="true"
              />
            </a-form-item>
            <div style="display: -webkit-inline-box">
              <a-form-item label="차인지급액">
                <number-box-money
                  :disabled="true"
                  v-model:valueInput="dataIncomeWageDaily.actualPayment"
                  width="160px"
                  :required="true"
                />
              </a-form-item>
          ...
          <a-col :span="10" style="padding-right: 5px">
            ...
            <div class="input-text red">
              <label>일급/월급:</label>
              <switch-basic
                :disabled="store.state.common.pa510.statusDisabledStatus"
                v-model:valueSwitch="
                  dataIncomeWageDaily.employee.monthlyPaycheck
                "
                :textUnCheck="'월급'"
              ...
              <number-box-money
                ...
                v-if="dataIncomeWageDaily.employee.monthlyPaycheck"
                width="110px"
                :required="true"
                placeholder="일급여"
                :spinButtons="false"
              ...
              <a-tooltip color="black" placement="top">
                <template
                  #title
                  v-if="dataIncomeWageDaily.employee.monthlyPaycheck"
                  >일급 선택시, 월급 = 일급 x 근무일수</template
                >
                <template #title v-else
                  >월급 선택시, 일급 = 월급 / 근무일수</template
                >
                <img src="@/assets/images/iconInfo.png" class="img-info" />
              </a-tooltip>
            </div>
            ...
            <div style="font-weight: bold">
              <span v-if="!dataIncomeWageDaily.employee.monthlyPaycheck"
                >일급여 {{ showDailyWage() }}원</span
              >
    ...
    >
      ...
      <a-tooltip placement="top">
        <template #title
          >4대보험 EDI 의 공제 금액이 있는 경우, 조회 후 적용합니다</template
        >
        <div>
          <button-basic
            :disabled="store.state.common.pa510.statusDisabledStatus || true"
            style="margin: 0px 5px"
            @onClick="actionInsurance"
            mode="contained"
            type="default"
            text="4대보험 EDI 조회/적용"
          />
