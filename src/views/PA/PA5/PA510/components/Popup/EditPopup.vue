<template>
      ...
      <standard-form action="" name="edit-510">
        <div class="custom-modal-edit">
          ...
          <date-time-box-custom
            ref="requiredDayValue"
            width="150px"
            :required="true"
            :startDate="startDate"
            :finishDate="finishDate"
            v-model:valueDate="dayValue"
          />
  ...
  <a-modal
    v-model:visible="statusOnCallApiChange"
    okText="확인"
    :closable="false"
    :footer="null"
  >
    ...
    <p class="d-flex-center">
      <img
        src="@/assets/images/changeDaySuccess.svg"
        alt=""
        class="mr-5"
      />처리건수: {{ sumSuccessCallApi }}건
    </p>
    <p class="d-flex-center">
      <img
        src="@/assets/images/changeDayErr.svg"
        alt=""
        class="mr-5"
      />미처리건수 및 내역: {{ sumErrorCallApi }} 건
    </p>
    <ul>
      <li
        v-for="item in arrDataError"
        :key="item.employeeId"
        class="list-error"
      >
        <employee-info
          :idEmployee="item.employee.employeeId"
          :name="item.employee.name"
          :idCardNumber="item.employee.residentId"
          :status="item.employee.status"
          :foreigner="item.employee.foreigner"
          :checkStatus="false"
        />
