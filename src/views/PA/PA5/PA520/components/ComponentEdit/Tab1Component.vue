    ...
    <standard-form formName="update-page-PA520" ref="formRefPa520Update">
      <a-form-item label="사번(코드)" label-align="right">
        <div class="d-flex-center">
          <text-number-box
            width="200px"
            v-model:valueInput="dataEdited.employeeId"
      ...
      <a-form-item label="성명" label-align="right" class="label-red">
        <default-text-box
          width="200px"
          v-model:valueInput="dataEdited.name"
          :required="true"
      ...
      <a-form-item label="입사년월일" label-align="right" class="joinedAt">
        <date-time-box
          width="150px"
          className="joinedAt"
          v-model:valueDate="dataEdited.joinedAt"
        />
      </a-form-item>
      <a-form-item label="퇴사년월일" label-align="right" class="leavedAt">
        <div class="input-text">
          <date-time-box
            width="150px"
            className="leavedAt"
            v-model:valueDate="dataEdited.leavedAt"
          />
      ...
      >
        <radio-group
          ...
          v-model:valueRadioCheck="dataEdited.foreigner"
      ...
      <div class="d-flex">
        ...
        >
          <country-code-select-box
            v-model:valueCountry="dataEdited.nationalityCode"
            :hiddenOptionKR="dataEdited.foreigner"
            width="180px"
            :disabled="disabledSelectBox || !isDeletable"
        ...
        >
          <stay-qualification-select-box
            v-model:valueStayQualifiction="dataEdited.stayQualification"
            :disabled="disabledSelectBox || !isDeletable"
            width="180px"
            :required="!disabledSelectBox"
          />
      ...
      <a-form-item :label="labelResident" label-align="right" class="label-red">
        <id-number-text-box
          width="150px"
          v-model:valueInput="dataEdited.residentId"
          :required="true"
          :foreigner="dataEdited.foreigner"
        />
      </a-form-item>
      <a-form-item label="주소정근무시간" class="label-red" label-align="right">
        <div class="input-text">
          <number-box
            :required="true"
            :spinButtons="true"
            v-model:valueInput="dataEdited.weeklyWorkingHours"
            width="150px"
            :min="1"
            :max="52"
          ></number-box>
      ...
      <a-form-item label="이메일" label-align="right">
        <div class="input-text">
          <mail-text-box
            width="200px"
            v-model:valueInput="dataEdited.email"
      ...
      <a-form-item label="부서" label-align="right">
        <select-search-edit
          v-model:valueInput="dataEdited.department"
      ...
      <a-form-item label="직위" label-align="right">
        <select-search-edit
          v-model:valueInput="dataEdited.responsibility"
  ...
  setup(props, { emit }) {
    ...
    getValueDefault((res) => {
      if (res.data) {
        ...
        dataEdited.nationalityCode =
          res.data.getEmployeeWageDaily.nationalityCode;
        dataEdited.stayQualification =
          res.data.getEmployeeWageDaily.stayQualification;
        dataEdited.residentId =
          res.data.getEmployeeWageDaily.residentId.replace("-", "");
        dataEdited.zipcode = "";
        ...
        dataEdited.retirementIncome =
          res.data.getEmployeeWageDaily.retirementIncome;
        dataEdited.weeklyWorkingHours =
          res.data.getEmployeeWageDaily.weeklyWorkingHours;
        dataEdited.department = res.data.getEmployeeWageDaily.department;
        dataEdited.responsibility =
          res.data.getEmployeeWageDaily.responsibility;
    ...
    const { mutate, onError, onDone } = useMutation(
      mutations.updateEmployeeWageDaily
    );
    onError((e) => {
        ...
        if (value == true) {
          ...
          dataEdited.nationalityCode =
            dataEdited.nationalityCode != "KR"
              ? dataEdited.nationalityCode
              : null;
          dataEdited.stayQualification = dataEdited.stayQualification
            ? dataEdited.stayQualification
            : null;
        } else {
