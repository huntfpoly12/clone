<template>
  <action-header
    title="기타소득자등록"
    @actionSave="disabledBlock ? false : actionSave()"
  ...
  <div id="pa-710">
    <div class="page-content">
      <a-row>
        <a-col
          span="16"
          class="data-table"
          :class="{ disabledBlock: disabledBlockTable }"
        >
          <a-spin :spinning="loading || loadingCreated" size="large">
            <DxDataGrid
              ...
              :data-source="listEmployeeExtra"
              :show-borders="true"
              key-expr="residentIdHide"
            ...
            >
              ...
              <DxColumn
                ...
                data-field="incomeTypeName"
              />
              <template #grade-cell="{ data }">
                <income-type
                  v-if="data.data.incomeTypeCode"
        ...
        <a-col
          span="8"
          class="custom-layout"
          :class="{ disabledBlock: disabledBlock }"
        >
          <a-spin :spinning="loadingDetail" size="large" :key="resetFormNum">
            <StandardForm formName="pa-710-form" ref="pa710FormRef">
              <a-form-item label="코드" :label-col="labelCol">
                <div class="d-flex-center">
                  <number-box
                    :required="true"
                    :width="200"
                    v-model:valueInput="formState.employeeId"
                    :disabled="true"
                  >
                  </number-box>
              ...
              <a-form-item label="내/외국인" :label-col="labelCol" class="red">
                <radio-group
                  :arrayValue="optionsRadio"
                  :required="true"
                  @update:valueRadioCheck="changeRadioForeigner"
                  v-model:valueRadioCheck="formState.foreigner"
              ...
              >
                <country-code-select-box
                  v-if="formState.foreigner"
                  style="width: 200px"
                  v-model:valueCountry="formState.nationalityCode"
                  @textCountry="textCountry"
                  :required="formState.foreigner"
              ...
              <a-form-item
                :label="
                  !formState.foreigner ? '주민등록번호' : '외국인번호 유효성'
                "
                :label-col="labelCol"
                class="red"
              >
              ...
              <a-form-item label="소득구분" :label-col="labelCol" class="red">
                <type-code-select-box
                  :screenCode="710"
                  style="width: 200px"
                  v-model:valueInput="formState.incomeTypeCode"
                  @textTypeCode="textTypeCode"
                  :required="true"
                  :disabled="statusFormUpdate"
                >
                </type-code-select-box>
              </a-form-item>
    ...
    />
    <PopupMessage
      :modalStatus="modalStatusAdd"
      @closePopup="modalStatusAdd = false"
      :typeModal="'confirm'"
      :title="Message.getMessage('COMMON', '501').message"
      content=""
      :okText="Message.getMessage('COMMON', '501').yes"
...
export default defineComponent({
  ...
  setup() {
    ...
    const contentDelete = ref(
      "선택된 소득자의 해당 원천년도에 소득 내역들이 있다면 삭제불가하며, 삭제한 후 복구불가합니다. 그래도 삭제하시겠습니까?"
    );
    // config grid
    ...
    // ================GRAPQL==============================================
    const {
      mutate: createEmployeeExtra,
      onDone: onDoneAdd,
      onError: onErrorAdd,
      loading: loadingCreated,
    } = useMutation(mutations.createEmployeeExtra);
    const {
      mutate: updateEmployeeExtra,
      onDone: onDoneUpdate,
      onError: onErrorUpdate,
    } = useMutation(mutations.updateEmployeeExtra);
    const { loading, result, onError } = useQuery(
      queries.getEmployeeExtras,
      originData,
      () => ({
    ...
    } = useMutation(mutations.deleteEmployeeExtra);
    onErrorDelete((error) => {
    ...
    onDoneAdd(async (data) => {
      ...
      if (statusClickButtonSave.value) {
        ...
        originDataDetail.value.employeeId =
          data.data.createEmployeeExtra?.employeeId;
    ...
    onDoneUpdate(async (data) => {
      ...
      if (statusClickButtonSave.value) {
        // if click submit
        originDataDetail.value.employeeId =
          data.data.updateEmployeeExtra?.employeeId;
      ...
      if (
        ...
        if (
          ...
          if (
            ...
            if (
              !statusAddRow.value &&
              listEmployeeExtra.value[listEmployeeExtra.value.length - 1]
                ?.employeeId == null
            ) {
              listEmployeeExtra.value = listEmployeeExtra.value.splice(
                0,
    ...
    const addRow = () => {
      ...
      listEmployeeExtra.value = JSON.parse(
        JSON.stringify(listEmployeeExtra.value)
      ).concat({ ...initialState });
      formState.value =
        listEmployeeExtra.value[listEmployeeExtra.value.length - 1];
      resetFormNum.value++;
    ...
    const changeRadioForeigner = (value: Boolean) => {
      if (!value) {
        ...
        formState.value.nationalityCode =
          formState.value.nationalityCode == "KR"
            ? null
            : formState.value.nationalityCode;
      }
      ...
      if (value) {
        ...
        if (runOne.value) {
          if (listEmployeeExtra.value.length) {
            originDataDetail.value.employeeId =
              listEmployeeExtra.value[0]?.employeeId;
