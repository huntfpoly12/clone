				...
				<a-col span="16" class="data-table" :class="{ disabledBlock: disabledBlockTable }">
					<a-spin :spinning="loading || loadingCreated" size="large">
						<DxDataGrid noDataText="내역이 없습니다" id="gridContainer" :show-row-lines="true"
							:hoverStateEnabled="true" :data-source="listEmployeeExtra" :show-borders="true"
							...
							<template #pupop="{ data }">
								<DeleteOutlined v-if="data.data.deletable" style="
                    font-size: 16px;
                    width: 100%;
                    height: 30px;
                    line-height: 30px;
					...
					<a-spin :spinning="loadingDetail" size="large" :key="resetFormNum">
						<StandardForm formName="pa-710-form" ref="pa710FormRef">
							...
							<a-form-item label="이메일" :label-col="labelCol">
								<div class="d-flex-center">
									<mail-text-box :width="200" placeholder="abc@example.com"
										v-model:valueInput="formState.email" id="email" />
									<a-tooltip color="black" placement="top">
										<template #title>원천징수영수증 등 주요서류를메일로 전달
											가능합니다.</template>
										<img src="@/assets/images/iconInfo.png" class="img-info" />
									</a-tooltip>
								</div>
