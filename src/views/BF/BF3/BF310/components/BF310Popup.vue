...
export default defineComponent({
    ...
    data() {
        return {
            ...
            value: ref<number>(1),
            dataSelectModal: '<button style="width:100%;height : 36px;text-align: left;background: white; border: 1px solid #d9d9d9; padding: 4px 6px; ">Select a value...</button>'
        };
    },
    components: {
        DxDropDownBox,
    ...
    methods: {
        ...
        onGridSelectionChanged(value: any) {
            ...
            this.gridDataSource.map(element => {
                if (element.ID == value) {
                    console.log(element);
                    html = '<div style="border: 1px solid #d9d9d9;padding: 6px;"><span style="border: 1px solid ' + this.getColorTag(element.심사상태) + ';color :' + this.getColorTag(element.심사상태) + '; padding:4px; border-radius: 5px; padding: 1px 5px;margin-right: 5px;">' + element.심사상태 + '</span>' + ' ' + element.상호 + ' ' + ' ' + element.신청일자 + ' ' + element.신청코드 + '</div>'
                }
            });
