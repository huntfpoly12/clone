...
export default defineComponent({
    ...
    setup() {
        ...
        let {
            ...
            onError: errorTable,
            onResult: resTable
        } = useQuery(queries.searchTaxWithholdingStatusReportsByImputedYearMonth, dataSearchTax, () => ({
            enabled: trigger.value,
            fetchPolicy: "no-cache"
        }));
        resTable(res => {
            dataSource.value = res.data.searchTaxWithholdingStatusReportsByImputedYearMonth
            trigger.value = false
        })
        errorTable(res => {
