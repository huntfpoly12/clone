      ...
      <a-spin :spinning="loadingIncomeBusinessPayment || loadingElectronicFilings" size="large">
            <DxDataGrid :show-row-lines="true" :hoverStateEnabled="true" :data-source="dataSource"
                ...
                <template #imputed="{data}"> 
                  <get-status-table v-if="data.data.lastProductionRequestedAt" :data="data.data" tabName="tab3"/>
                </template>
            </DxDataGrid>
        </a-spin>
    </div>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    watch(resIncomeBusinessPayment, (value) => {
      if (value) {
        dataSource.value = [value.searchIncomeBusinessPaymentStatementElectronicFilings[0]]
