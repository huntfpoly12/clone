  ...
  <div id="bf-630" class="mt-10">
    <a-tabs v-model:activeKey="activeKey" type="card" style="overflow: visible !important;">
      ...
      <a-tab-pane :key="2" tab="퇴직소득지급명세서">
        <tab-2-component :activeSearch="activeSearch2" v-if="activeKey == 2"></tab-2-component>
      </a-tab-pane>
      <a-tab-pane :key="3" tab="거주자사업소득지급명세서">
        <tab-3-component :activeSearch="activeSearch3" v-if="activeKey == 3"></tab-3-component>
      </a-tab-pane>
      <a-tab-pane :key="4" tab="거주자기타소득지급명세서">
        <tab-4-component :activeSearch="activeSearch4" v-if="activeKey == 4"></tab-4-component>
      </a-tab-pane>
      <a-tab-pane :key="5" tab="전자신고파일제작내역">
        <tab-5-component :activeSearch="activeSearch5" :active-save="activeSavingTab5"
          v-if="activeKey == 5"></tab-5-component>
      </a-tab-pane>
    </a-tabs>
  </div>
</template>
