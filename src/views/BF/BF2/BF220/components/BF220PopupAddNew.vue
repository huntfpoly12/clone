                ...
                <a-row :gutter="24">
                    ...
                    <a-col :span="16">
                        <a-form-item label="대상회원">
                            <a-radio-group v-model:value="dataRes.type" @change="changeTypeUser">
                                <a-radio value="m">
    ...
    setup(props) {
        ...
        watch(resList, (value) => {
            if (value.isScreenRoleGroupRegistableId == true) {
                message.success(`ID này có thể sử dụng`)
            } else {
                message.error(`Id này đã được sử dụng`)
            }
        });
        ...
        const changeTypeUser = () => {
            console.log(dataRes.value.type);
