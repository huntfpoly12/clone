<template>
	<a-spin :spinning="loadingWithholdingConfig">
		...
		<standard-form formName="add-cm-310" class="standard-form" ref="formRefTab2">
			<a-spin :spinning="loadingDetail" :key="resetFormNum">
				<h2 class="title_modal">
					{{ statusFormUpdate ? "급여상세항목" : "급여항목추가" }}
				</h2>
				<div v-if="!statusFormUpdate">
					...
					<div class="mt-20" style="margin-left: 230px">
						<button-basic class="button-form-modal" :text="'저장'" :type="'default'" :mode="'contained'"
							@onClick="actionSave" />
					</div>
				</div>
				<div v-else>
		...
		<ConfirmDeletePopup :modalStatus="modalRowDelete" @closePopup="modalRowDelete = false" :contentYes="'네'"
			:contentNo="'아니요'" @confirmDelete="confirmDelete">삭제하겠습니까?</ConfirmDeletePopup>
	</a-spin>
</template>
<script lang="ts">
...
export default defineComponent({
	...
	setup() {
		...
		// get config
		const dataQueryConfigPayItems = ref({
			companyId: companyId,
			imputedYear: globalYear,
		});
		const dataQueryConfigPayItem = ref<any>({
			companyId: companyId,
			imputedYear: globalYear,
			itemCode: null,
		});
		...
		const {
			result: resultWithholdingConfig,
			loading: loadingWithholdingConfig,
		} = useQuery(
			queries.getWithholdingConfigPayItems,
			dataQueryConfigPayItems,
			() => ({
		...
		const { result: resultConfigPayItem, loading: loadingDetail } = useQuery(
			queries.getWithholdingConfigPayItem, dataQueryConfigPayItem, () => ({
		...
		const {
			...
			onDone: onDoneDelete,
			onError: errorDelete,
		} = useMutation(mutations.deleteWithholdingConfigPayItem);
		...
		onDoneAdd(async (res) => {
			...
			if (statusClickButtonSave.value) {
				// if click submit
				dataQueryConfigPayItem.value.itemCode =
					res.data.createWithholdingConfigPayItem?.itemCode;
			} else {
				// if click save modal
		...
		onDoneUpdated(async (res) => {
			...
			if (statusClickButtonSave.value) {
				// if click submit
				dataQueryConfigPayItem.value.itemCode =
					res.data.updateWithholdingConfigPayItem?.itemCode;
			} else {
				// if click save modal
				dataQueryConfigPayItem.value.itemCode = dataRow.itemCode;
			}
		...
		const onFocusedRowChangingTab2 = (e: any) => {
			...
			if (dataRow.itemCode && dataRow.itemCode != formState.value.itemCode) {
				dataQueryConfigPayItem.value.itemCode =
					e.rows[e.newRowIndex]?.data.itemCode;
				if (
					statusFormUpdate.value == false &&
					JSON.stringify(initialState) !== JSON.stringify(formState.value)
				) {
					...
					if (
						JSON.stringify(dataRowOld) !== JSON.stringify(formState.value) &&
						statusFormUpdate.value == true
					) {
						modalStatus.value = true;
						rowElement?.classList.add("dx-state-hover-custom");
						...
						if (
							!statusAddRow.value &&
							dataSource.value[dataSource.value.length - 1]?.employeeId == null
						) {
				...
				if (statusFormUpdate.value) {
					...
					let variables = {
						...
						input: {
							...
							taxfreePayItemCode:
								formState.value.taxPayCode[0] === "비과세"
									? formState.value.taxPayCode[1]
									: null,
							taxPayItemCode:
								formState.value.taxPayCode[0] === "과세"
									? formState.value.taxPayCode[1]
									: null,
						},
					};
		...
		const deleteConfig = (data: any) => {
			dataItemCode.value = data.data.itemCode
		...
		const openAddNewModal = () => {
			if (dataSource.value.length <= 20) {
				let statusChangeFormAdd =
					JSON.stringify({ ...initialState }) !==
					JSON.stringify(formState.value) && statusFormUpdate.value == false;
				let statusChangeFormEdit =
					JSON.stringify(dataRowOld) !== JSON.stringify(formState.value) &&
					statusFormUpdate.value == true;
				if (statusChangeFormEdit) {
					// if status form add and form not null
					modalStatusAdd.value = true;
					statusClickButtonAdd.value = true;
				} else {
					if (statusChangeFormAdd) {
						// if status form add and form not null
						modalStatusAdd.value = true;
						statusClickButtonAdd.value = true;
				...
				notification(
					"error",
					`이용 가능한 급여항목은 최대 20개입니다. 기존항목을 이용중지한 후 새로 추가하세요`
				);
			}
		};
		...
		const addRow = () => {
			...
			dataSource.value = JSON.parse(JSON.stringify(dataSource.value)).concat({
				...initialState,
			});
