...
export default defineComponent({
  ...
  setup() {
    ...
    const listFacilityBizTypeForUser = computed(
      () => store.state.settings.listFacilityBizTypeForUser
    );
    ...
    const isInputWebID = computed(() => {
      return (
        dataDetailBankbook.value.bankbookInput.useScrap &&
        ((!!inputIDPWBankType.value.corporate.ID &&
          isTypeClassification.value) ||
          (!!inputIDPWBankType.value.private.ID && !isTypeClassification.value))
      );
    });
    const isInputWebPW = computed(() => {
      return (
        dataDetailBankbook.value.bankbookInput.useScrap &&
        ((!!inputIDPWBankType.value.corporate.PW &&
          isTypeClassification.value) ||
          (!!inputIDPWBankType.value.private.PW && !isTypeClassification.value))
      );
    });
    ...
    watch(
      ...
      (value) => {
        ...
        if (
          !isEqual(oldDataDetailBankbook.value, {
            ...dataDetailBankbook.value,
            fiscalYear: globalYearSelected.value,
          }) ||
          (isCreate.value && !!dataDetailBankbook.value.facilityBusinessId)
        ) {
      ...
      if (value.getBankbooks) {
        ...
        if (isNewCreate.value) {
          ...
          if (isChangeFocusSubmit.value && !isStatusClickCreate.value) {
            ...
            paramBankbookDetail.bankbookId =
              value.getBankbooks[lengthData].bankbookId;
            paramBankbookDetail.facilityBusinessId =
              value.getBankbooks[lengthData].facilityBusinessId;
          }
          if (isChangeFocusSubmit.value && isStatusClickCreate.value) {
        ...
        if (isDelete.value) {
          if (value.getBankbooks.length !== 0) {
            if (value.getBankbooks.length - 1 < indexRow.value) {
              focusedRowKey.value =
                value.getBankbooks[indexRow.value - 1].bankbookId;
              paramBankbookDetail.facilityBusinessId =
                value.getBankbooks[indexRow.value - 1].facilityBusinessId;
              paramBankbookDetail.bankbookId =
                value.getBankbooks[indexRow.value - 1].bankbookId;
            } else {
              focusedRowKey.value =
                value.getBankbooks[indexRow.value].bankbookId;
              paramBankbookDetail.facilityBusinessId =
                value.getBankbooks[indexRow.value].facilityBusinessId;
              paramBankbookDetail.bankbookId =
                value.getBankbooks[indexRow.value].bankbookId;
            }
            triggerBankbook.value = true;
    ...
    watch(resBankbook, (value) => {
      ...
      isCreateduseScrap.value = cloneDeep(
        dataDetailBankbook.value.bankbookInput.useScrap
      );
    ...
    watch(
      () => dataDetailBankbook.value.bankbookInput.type,
      (value) => {
        if (value) {
          const typeItem: any = bankTypeCommon.find(
            (item: any) => item.c == value
          );
    ...
    watch(
      () => dataDetailBankbook.value.bankbookInput.useType,
      (value) => {
        if (isCheckAdding.value) {
    ...
    watch(
      () => dataDetailBankbook.value.bankbookInput.bankbookNickname,
      (value) => {
        isDuplicaseName.value = !dataSource.value.some(
          (items, index) =>
            index !== indexRow.value && items.bankbookNickname === value
        );
      }
    );
    ...
    const onReorder = (e: any) => {
      ...
      const toIndex = dataSource.value.findIndex(
        (item: any) =>
          item.bankbookId === visibleRows[e.toIndex].data.bankbookId
      );
      const fromIndex = dataSource.value.findIndex(
        (item: any) => item.bankbookId === e.itemData.bankbookId
      );
    ...
    const dataRegisterBankbook = (data: any) => {
      if (
        ...
        dataSource.value[dataSource.value.length - 1].bankbookId === newSampleID
      ) {
    ...
    const resetDataDetail = () => {
      ...
      dataDetailBankbook.value.bankbookInput.sort = dataSource.value.length
        ? dataSource.value[dataSource.value.length - 1].sort + 1
        : 0;
    ...
    const submit = () => {
      ...
      if (isTypeClassification.value) {
        ...
        data.scrapingInfoInput.birthday = parseInt(
          data.scrapingInfoInput.birthday
        );
      }
      ...
      if (isCreate.value) {
        ...
        if (dataDetailBankbook.value.bankbookInput.useScrap) {
          if (!data.scrapingInfoInput?.accountPassword)
            delete data.scrapingInfoInput.accountPassword;
          if (!data.scrapingInfoInput?.bizNumber)
            delete data.scrapingInfoInput.bizNumber;
          if (!data.scrapingInfoInput?.birthday)
            delete data.scrapingInfoInput.birthday;
          if (!data.scrapingInfoInput?.webId)
            delete data.scrapingInfoInput.webId;
          if (!data.scrapingInfoInput?.webPassword)
            delete data.scrapingInfoInput.webPassword;
        }
        if (!isRequiredAccountPassword.value) {
          delete data.scrapingInfoInput;
        }
        updateBankbook(data);
      }
    ...
    const onFocusedRowChanging = (event: any) => {
      ...
      if (
        !isEdited.value ||
        event.rows[event.prevRowIndex].data.bankbookId === newSampleID
      ) {
        event.cancel = true;
        if (event.prevRowIndex !== event.newRowIndex) {
          const rowElement = document.querySelector(
            `[aria-rowindex="${event.newRowIndex + 1}"]`
          );
        ...
        if (
          paramBankbookDetail.bankbookId ===
          event.rows[event.newRowIndex].data.bankbookId
        )
          return;
        indexRow.value = event.newRowIndex;
        paramBankbookDetail.facilityBusinessId =
          event.rows[event.newRowIndex].data.facilityBusinessId;
        paramBankbookDetail.bankbookId =
          event.rows[event.newRowIndex].data.bankbookId;
        triggerBankbook.value = true;
      }
    ...
    const handleConfirmChange = (val: boolean) => {
      if (val) {
        ...
        if (isChangeGlobalYear.value) {
          ...
          const isCheckAdding =
            dataSource.value[dataSource.value.length - 1].bankbookId ===
            newSampleID;
          if (isCheckAdding) {
            dataSource.value = dataSource.value.filter(
              (items: any) => items.bankbookId !== newSampleID
            );
          }
          if (isStatusClickCreate.value) {
            ...
            if (isCheckAdding) {
              ...
              paramBankbookDetail.facilityBusinessId =
                dataSource.value[index].facilityBusinessId;
              paramBankbookDetail.bankbookId =
                dataSource.value[index].bankbookId;
            }
            isModalRegister.value = true;
          } else {
            indexRow.value = objChange.value.indexRow;
