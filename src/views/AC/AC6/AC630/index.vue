<template>
      ...
      <a-row>
        <a-col :span="8">
          <a-form-item label="기부기간" label-align="right" class="red">
            <div class="input-text">
              <year-picker-box
                width="100px"
                v-model:valueDate="dataState.imputedYear"
                color="#a6a6a6"
              ></year-picker-box>
              <span> (1/1~12/31) </span>
            </div>
        ...
        <a-col :span="8">
          ...
          <div style="font-size: 12px; color: #888888" class="mt-5">
            <img src="@/assets/images/iconInfo.png" style="width: 14px" />본
            회계연도내 기부내역 대상의 발행내역만 검색가능하며, 발행기간과는
            무관합니다.
          </div>
        </a-col>
        <a-col :span="8">
          <a-form-item label="후원자" label-align="right">
            <default-text-box
              width="200px"
              v-model:valueInput="dataState.name"
            />
          </a-form-item>
        </a-col>
      </a-row>
        ...
        >
          ...
          <template #button-add>
            <a-tooltip placement="top" class="custom-tooltip">
              <template #title>
                <div style="width: 100px">기부금영수증 신규 발행</div>
              </template>
              <div style="text-align: center">
  ...
  <PopupMessage
    ...
    okText="네. 삭제합니다"
    cancelText="아니요"
    @checkConfirm="onDeleteModal"
  />
  <CreatePopup v-if="modalCreate" @onCreateModal="onCreateModal" />
  <ShowDataCreated
    v-if="modalShowCreated"
    @onShowCreateModdal="onShowCreateModdal"
  />
</template>
<script lang="ts">
...
export default defineComponent({
  ...
  setup() {
    ...
    const rangeDate: any = ref([
      parseInt(dayjs().subtract(1, "week").format("YYYYMM")),
    ...
    const deleteContent = h("div", [
      h("div", `선택된 기부금영수증 ${1} 건을 삭제합니다.`),
      h("div", "삭제 후 복구는 불가합니다. 그래도 삭제하시겠습니까?"),
    ]);
