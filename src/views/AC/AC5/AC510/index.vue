            ...
            <div class="flex">
                <a-form-item label="서식 설정">
                    <div class="dflex custom-flex">
                        ...
                        <checkbox-basic v-model:valueCheckbox="dataQueryGetAccountBookReportViewUrl.displayCode" :size="'20'" />
...
export default defineComponent({
    ...
    setup() {
        ...
        const {
            onResult: resGetAccountBookReportViewUrl, loading: loadingGetAccountBookReportViewUrl, onError: errorGetAccountBookReportViewUrl
        } = useQuery(queries.getAccountBookReportViewUrl, dataQueryGetAccountBookReportViewUrl.value, () => ({
        ...
        const actionPrint = (data: any) => {
            // gridRef.value?.instance.deselectAll()
            dataQueryGetAccountBookReportViewUrl.value.year = dataGetAccountingProcesses.value?.find((item: any) => item.month === monthSelected.value)?.year
            dataQueryGetAccountBookReportViewUrl.value.type = data.type
            triggerGetAccountBookReportViewUrl.value = true;
        }
        return {
            acYear, monthSelected, selectedMonth,
            dataQueryGetAccountBookReportViewUrl,
            move_column,
            colomn_resize,
            dataGetAccountingProcesses, loadingGetAccountingProcesses,
            // valueCheckBox,
            dataSourceAC510,
            actionPrint,
