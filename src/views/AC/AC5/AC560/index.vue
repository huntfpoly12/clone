  ...
  <div class="ac-560">
    ...
    <div class="ac-560-formatting">
      ...
      <div class="ac-560-formatting-option">
        ...
        <checkbox-basic v-model:valueCheckbox="displayMonthlySummary" class="mr-10" label="월별 합계 금액 및 건수"
                        :disabled="false" :size="'20'"/>
        <a-tooltip color="black" placement="top">
...
import queries from "@/graphql/queries/AC/AC5/AC560";
...
const accountingReportType = ref(11)
...
onResAccountingProcesses((data: any) => {
  if (data.data.getAccountingProcesses.length > 0) {
    ...
    if (data.data.getAccountingProcesses.length === 1) {
      maxMonth.value = data.data.getAccountingProcesses[0].month
      maxStartMonth.value = data.data.getAccountingProcesses[0].month
      endMonth.month = data.data.getAccountingProcesses[0].month
      endMonth.status = data.data.getAccountingProcesses[0].status
    } else {
...
watchEffect(() => {
  if (startMonth.month && endMonth.month) {
    ...
    startMonth.yearMonth = fomatYearMonth(acYear.value, startMonth.month)
    endMonth.yearMonth = fomatYearMonth(acYear.value, endMonth.month)
  }
})
