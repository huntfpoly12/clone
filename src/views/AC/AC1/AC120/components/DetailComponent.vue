<template>
  <div class="column">
    <div class="detail">
      ...
      <a-row
        class="detail-body"
        :class="store.state.common.ac120.statusShowFull ? 'showFull' : ''"
      >
        <a-col class="detail1">
          <StandardForm
            formName="ac-120-form"
            ref="refFormAC120"
            :key="store.state.common.ac120.keyRefreshForm"
          >
            <div class="ac120">
              <a-row class="mt-20">
                <a-col :span="7" class="col-1">
                  ...
                  <div class="input_info">
                    ...
                    <button-basic
                      @onClick="actionOpenModalCopy"
                      style="margin: -5px 0px 0px 5px"
                      mode="contained"
                      type="default"
                      :text="textButton + '으로 변경'"
                    />
                  </div>
                </a-col>
                <a-col :span="5" class="col-2">
                  <a-form-item label="결의일자" class="red">
                    <date-time-box
                      :teleport="true"
                      v-model:valueDate="formData.transactionDetailDate"
                      width="130px"
                      :required="true"
                      disabled="true"
                    />
                ...
                <a-col :span="6" class="col-3">
                  <a-form-item label="금액" class="red">
                    <number-box-money
                      v-model:valueInput="formData.amount"
                      width="150px"
                      :required="true"
                      :spinButtons="false"
                      disabled="true"
                  ...
                  <a-form-item label="적요" class="red">
                    <default-text-box
                      v-model:valueInput="formData.summary"
                      width="150px"
                      :required="true"
                    />
                  </a-form-item>
                ...
                <a-col :span="6" class="col-4 fl-r">
                  <a-form-item label="계정과목" class="red">
                    <account-code-select
                      :key="resetSelectAccount"
                      :classification="classificationAccountCode"
                      v-model:valueInput="formData.accountCode"
                      width="230px"
                      :required="true"
                    />
                  </a-form-item>
                ...
                <a-col :span="7" class="col-1">
                  ...
                  >
                    <date-time-box
                      :teleport="true"
                      v-model:valueDate="formData.causeActionDate"
                      width="150px"
                      :required="true"
                  ...
                  <a-form-item label="결재일자">
                    <date-time-box
                      :teleport="true"
                      v-model:valueDate="formData.paymentDate"
                      width="150px"
                    />
                  </a-form-item>
                  <a-form-item label="발의일자">
                    <date-time-box
                      :teleport="true"
                      v-model:valueDate="formData.proposedDate"
                      width="150px"
                    />
                  </a-form-item>
                </a-col>
                <a-col :span="5" class="col-2">
                  ...
                  <a-form-item label="출납일자">
                    <date-time-box
                      :teleport="true"
                      v-model:valueDate="formData.accountingDate"
                      width="130px"
                    />
                  </a-form-item>
                ...
                <a-col :span="6" class="col-3">
                  <a-form-item label="거래처">
                    <customer-select
                      :disabled="formData.resolutionClassification == 1"
                  ...
                  <div class="input_info">
                    <a-form-item :label="textLabelInputSource">
                      <default-text-box
                        v-model:valueInput="formData.source"
                        width="150px"
                      />
                    </a-form-item>
                    <a-tooltip
                      placement="top"
                      color="black"
                      class="fz-10 ml-10 mb-5"
                    >
                      <template #title
                        >기본값은 [회계설정 > 회계기타] 메뉴에서 입력된 결의서
                  ...
                  <div class="input_info">
                    <a-form-item label="작성자">
                      <default-text-box
                        v-model:valueInput="formData.writer"
                        width="150px"
                      />
                    </a-form-item>
                    <a-tooltip
                ...
                <a-col :span="6" class="col-4 fl-r">
                  ...
                  <a-form-item label="메모">
                    ...
                    <text-area-box
                      width="230px"
                      v-model:valueInput="formData.memo"
                      :height="60"
                    />
                  </a-form-item>
                </a-col>
              ...
              <a-row>
                ...
                <a-col :span="12">
                  <a-form-item label="물품내역수">
                    <default-text-box
                      :disabled="true"
                      v-model:valueInput="formData.goodsCount"
                      width="150px"
                    />
                  </a-form-item>
                </a-col>
              ...
              <a-row>
                <a-col :span="24">
                  <a-form-item label="품의 원인 및 용도">
                    <text-area-box
                      :disabled="formData.resolutionClassification == 1"
                      v-model:valueInput="formData.causeUsage"
                      :height="50"
                    />
                  </a-form-item>
                </a-col>
          ...
          <div class="text-align-center mt-20">
            <DxButton
              @click="onCancelDeleteRow"
              class="ml-4 custom-button-checkbox custom-button"
              type="default"
              :height="$config_styles.HeightInput"
            >
  ...
  <PopupCopyData
    :modalStatus="statusPopupCopyData"
    @closePopup="statusPopupCopyData = false"
    @submit="statusPopupCopyData = false"
  />
  <ModalDelete
    :modalStatus="statusModalDelete"
    @closePopup="statusModalDelete = false"
    :dataRows="[formData]"
  />
  <PopupMessage
    :modalStatus="isModalChange"
    @closePopup="isModalChange = false"
    :typeModal="'confirm'"
    :title="''"
    :content="Message.getMessage('AC120', '001').message"
    :okText="Message.getMessage('AC120', '001').yes"
    :cancelText="Message.getMessage('AC120', '001').no"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const textButton = ref<string>(
      store.state.common.ac120.arrResolutionType.find(
        (element: any) => element.id == 22
      )?.text
    );
    const textLabelInputSource = ref<string>("수입원");
    const requiredCauseActionDate = ref();
    const arrayRadioCheck = computed(
      () => store.state.common.ac120.arrLetterOfApprovalType
    );
    let colorDate = ref();
