  ...
  setup(props, { emit }) {
    ...
    watch(resGetBankbookDetailProofs, (value) => {
      if (value.getBankbookDetailProofs.length) {
        listFileStorageId.value = value.getBankbookDetailProofs.map(
          (file: any) => ({
            ...file,
    ...
    const remove = (e: any) => {
      if (e.status === "error") {
        const index = listFileStorageId.value.findIndex(
          (img: any) => img.uid === e.uid
        );
        listFileStorageId.value.splice(index, 1);
        return true;
      }
      indexImgRemove.value = listFileStorageId.value.findIndex(
    ...
    const handleDelete = (status: boolean) => {
      if (status) {
        ...
        removeBankbookDetailProof({
          ...props.payLoadProofs,
          fileStorageId:
            listFileStorageId.value[indexImgRemove.value].fileStorageId,
        });
      }
      isModalDelete.value = false;
    };
