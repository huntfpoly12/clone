    ...
    <a-spin :spinning="loading">
      <a-radio-group v-model:value="choose">
        ...
        >
          ...
          <DxColumn caption="" cell-template="radio" width="40px" alignment="center"/>
          ...
          <DxColumn data-field="manageStartDate" caption="관리시작일" alignment="center" cell-template="manageStartDate"/>
          ...
          <template #radio="{data}">
            <a-radio :value="data.data.code"/>
          </template>
          <template #code="{data}">
            {{ data.data.code }}
            <a-tag v-if="!data.data.active" color="#DC5939">해지</a-tag>
          </template>
          <template #manageStartDate="{data}">{{data.data.manageStartDate && filters.formatDate(data.data.manageStartDate)}}</template>
        </DxDataGrid>
      </a-radio-group>
...
const onFocusedRowChanged = (e: FocusedRowChangedEvent) => {
  if (e?.row?.data && openRowCompanyTab3) {
    openRowCompanyTab3({
