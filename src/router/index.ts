...
import {AdminScreenRole, getJwtObject, ScreenRole, WorkScreenRole} from "@bankda/jangbuda-common";
...
const routes = [
  ...
  },
  {
    ...
    meta: {
      needAuth: true,
      roles: Object.values(AdminScreenRole.all())
    },
    children: [
      {
        path: "bf-310",
        ...
        meta: {
          roles: [AdminScreenRole.CONTRACT_MANAGE]
        }
      },
      {
        path: "bf-320",
        ...
        meta: {
          roles: [AdminScreenRole.COMPANY_MANAGE]
        }
      },
      {
        path: "bf-330",
        ...
        meta: {
          roles: [AdminScreenRole.SERVICE_MANAGE]
        }
      },
      {
        path: "bf-340",
        ...
        meta: {
          roles: [AdminScreenRole.SALES_REPRESENTATIVE_MANAGE]
        }
      },
      {
        path: "bf-210",
        ...
        meta: {
          roles: [AdminScreenRole.USER_MANAGE]
        }
      },
      {
        path: "bf-220",
        ...
        meta: {
          roles: [AdminScreenRole.ROLE_MANAGE]
        }
      },
      {
        path: "bf-610",
        ...
        meta: {
          roles: [AdminScreenRole.WITHHOLDING_WORK_CLOSING_MANAGE]
        }
      },
      {
        path: "bf-620",
      ...
      },
      {
        ...
        meta: {
          roles: [WorkScreenRole.BASIC_SETTINGS]
        }
      },
      {
        path: "cm-121",
      ...
      },
      {
        ...
        meta: {
          roles: [WorkScreenRole.WITHHOLDING_SETTINGS]
        }
      },
      {
        path: "pa-110",
        ...
        meta: {
          roles: [WorkScreenRole.WITHHOLDING_WORK]
        }
      },
      {
        path: "pa-120",
      ...
      },
      {
        ...
        meta: {
          roles: [WorkScreenRole.ACCOUNTING_WORK]
        }
      },
      {
        path: "ac-120",
...
router.beforeEach((to, from, next) => {
  ...
  const roles = isEmpty(to.meta.roles) ? null : to.meta.roles as ScreenRole[]
  const token = sessionStorage.getItem("token");
  const jwtObject = token && getJwtObject(token);
  let isRead = false;
  if (jwtObject) {
    isRead = Boolean(roles?.some((role: ScreenRole) => jwtObject.hasReadScreenRole(role)))
  }
  !roles && (isRead = true)
  if (requiresAuth && !token) {
    next("/login");
  } else if (requiresAuth && token && !isRead) {
    next("not-found");
  } else {
    next()
  }
